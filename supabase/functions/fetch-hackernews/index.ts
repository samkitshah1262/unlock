// HackerNews Content Fetcher
// Fetches top tech articles and parses with LLM
...
serve(async (req) => {
  try {
    ...
    // 2. Get first 200 stories (we will randomize later)
    ...
    // 3. Filter stories with URLs and good scores
    const validStories = stories.filter(
      (s) => s.url && s.score > 100 && s.type === 'story'
    )

    console.log(`Found ${validStories.length} valid HN stories`)

    // 4. Randomize and take up to 3 (reduced to avoid timeout)
    ...
    // 5. Create or resume job with checkpoint support
    ...
    for (const story of toProcess) {
      ...
      try {
        ...
        // Scrape with Firecrawl (includes retry, captcha detection)
        ...
        // Handle captcha/blocking - pause job
        ...
        // Get content from Firecrawl (markdown preferred, fallback to HTML)
