// TechCrunch Fetcher via RSS
// Parses recent posts and summarizes with LLM
...
serve(async () => {
  try {
    ...
    const { jobId, checkpoint } = await getOrCreateJob('techcrunch', urls, supabase)
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (!scrapeResult.success && (scrapeResult.errorCode === 'CAPTCHA' || scrapeResult.errorCode === 'BLOCKED')) {
          return new Response(
            JSON.stringify({
              ...
              message: `Job paused due to ${scrapeResult.errorCode}`,
              processed,
              failed,
              jobId
            }),
        ...
        if (!scrapeResult.success) {
          console.error(`Failed to scrape ${item.link}: ${scrapeResult.error}`)
          failed++
          continue
        }
        ...
        console.error('TechCrunch item error:', err)
      }
    }
